...
{
	$query = "
	SELECT library_timezone FROM ims_setting 
	LIMIT 1
	";
...
function get_one_day_fines($connect)
{
	$output = 0;
	$query = "
	SELECT library_one_day_fine FROM ims_setting 
	LIMIT 1
	";
	$result = $connect->query($query);
	foreach($result as $row)
...
// 	$output = '';
// 	$query = "
// 	SELECT library_currency FROM ims_setting 
// 	LIMIT 1
// 	";
// 	$result = $connect->query($query);
// 	foreach($result as $row)
...
function get_book_issue_limit_per_user($connect)
{
	$output = '';
	$query = "
	SELECT library_issue_total_book_per_user FROM ims_setting 
	LIMIT 1
	";
	$result = $connect->query($query);
	foreach($result as $row)
...
function get_total_book_issue_per_user($connect, $user_unique_id)
{
	...
	$query = "
	SELECT COUNT(issue_book_id) AS Total FROM ims_issue_book 
...
function get_total_book_issue_day($connect)
{
	...
	$query = "
	SELECT library_total_book_issue_day FROM ims_setting 
	LIMIT 1
	";
...
{
	$query = "
	SELECT author_name FROM ims_author 
...
{
	$query = "
	SELECT category_name FROM ims_category 
...
{
	$query = "
	SELECT location_rack_name FROM ims_location_rack 
...
function Count_total_issue_book_number($connect)
{
	...
	$query = "SELECT COUNT(issue_book_id) AS Total FROM ims_issue_book";
...
function Count_total_fines_received($connect)
{
	...
	$query = "
	SELECT SUM(book_fines) AS Total FROM ims_issue_book 
	WHERE book_issue_status = 'Return'
	";
...
function Count_total_book_number($connect)
{
	...
	$query = "
	SELECT COUNT(book_id) AS Total FROM ims_book 
...
function Count_total_author_number($connect)
{
	...
	$query = "
	SELECT COUNT(author_id) AS Total FROM ims_author 
	WHERE author_status = 'Enable'
	";
...
function Count_total_category_number($connect)
{
	...
	$query = "
	SELECT COUNT(category_id) AS Total FROM ims_category 
	WHERE category_status = 'Enable'
	";
...
function Count_total_location_rack_number($connect)
{
	...
	$query = "
	SELECT COUNT(location_rack_id) AS Total FROM ims_location_rack 
	WHERE location_rack_status = 'Enable'
	";
